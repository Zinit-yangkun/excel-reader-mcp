...
export function createXlsWithImages(testDir: string): string {
  ...
  const imageSpContainer = writeEscherRecord(0xf004, 0x0f, 0,
    Buffer.concat([spRecord, optRecord, clientAnchor, clientDataRecord])
  );

  // SpGR container wrapping both shape containers
  const spgrContainer = writeEscherRecord(0xf003, 0x0f, 0,
    Buffer.concat([groupSpContainer, imageSpContainer])
  );

  // DgContainer
  const dgContainer = writeEscherRecord(0xf002, 0x0f, 0,
    Buffer.concat([dgAtom, spgrContainer])
  );
